...
public class NewJLabel extends JLabel {
    ...
    //function called if we click on a Newjlabel
	public void JlabelClicked(NewJLabel label) throws InterruptedException {
						...
						.setIcon(
								window.reSize(
										new ImageIcon(System.getProperty("user.dir")
												+ "\\resources\\images\\alibicards\\" + "ALIBI_" + name + ".png"),
		...
		if (label.getMatrice_position()[0] == 1 && Game.getTurnCount() > 0 && window.actionPlaying == false) {
			if (jackgame.getActionTokenList().get(label.getMatrice_position()[1]).hasBeenPlayed() == false) {
				...
				label.setIcon(
						window.reSize(new ImageIcon(greyImage + ".png"), window.allSize[1][0], window.allSize[1][1]));

				if (jackgame.getActionTokenList().get(label.getMatrice_position()[1]).isRecto()) {

					jackgame.playAction(jackgame.actionGetFromList(window,
							jackgame.getActionTokenList().get(label.getMatrice_position()[1]).getAction1().toString(),
							label.getMatrice_position()[1], jackgame), window, jackgame);
				} else {

					jackgame.playAction(jackgame.actionGetFromList(window,
							jackgame.getActionTokenList().get(label.getMatrice_position()[1]).getAction2().toString(),
							label.getMatrice_position()[1], jackgame), window, jackgame);
				}
		...
		// called if we click on a swapAction
		if (label.getMatrice_position()[0] == 2 && window.swapDistrict) {
			...
			if (window.swapPosition[0][0] == x && window.swapPosition[0][1] == y) {
				...
				label.setIcon(
						window.reSize(
								PaintJLabel.imageIconsuperposer(
										RotateImage.rotateImage(window.reSize(new ImageIcon(label.getPath()),
										...
										window.reSize(new ImageIcon(label.getPathtwo()), window.allSizeCopy[6][0],
												window.allSizeCopy[6][1])),
								window.allSize[2][0], window.allSize[2][0]));
			} else {
				window.swapPosition[window.compteur_District][0] = x;
				window.swapPosition[window.compteur_District][1] = y;
				label.setIcon(
						window.reSize(
								PaintJLabel.imageIconsuperposerTwo(
										PaintJLabel.imageIconsuperposer(
												RotateImage.rotateImage(
														window.reSize(new ImageIcon(label.getPath()),
																window.allSizeCopy[2][0], window.allSizeCopy[2][0]),
														label.getAngle()),
												window.reSize(new ImageIcon(label.getPathtwo()),
														window.allSizeCopy[6][0], window.allSizeCopy[6][1])),
										window.reSize(
												new ImageIcon(System.getProperty("user.dir")
														+ "\\resources\\images\\districts\\selectedDistrict.png"),
												window.allSizeCopy[2][0], window.allSizeCopy[2][0])),
								window.allSize[2][0], window.allSize[2][0]));
				window.compteur_District++;
			}
			if (window.compteur_District == 2) {
				window.changeImage(window.matrice[window.swapPosition[0][0]][window.swapPosition[0][1]],
						window.matrice[window.swapPosition[1][0]][window.swapPosition[1][1]]);
				window.swapDistrict = false;
				window.compteur_District = 0;
				jackgame.swap(window.transformCoordToList(window.swapPosition[0]),
						window.transformCoordToList(window.swapPosition[1]));
		...
		// called if we click on a rotate action
		if (label.getMatrice_position()[0] == 2 && window.rotateDistrict) {
			if (((District) jackgame.getBoard().getCell(window.transformCoordToList(label.getMatrice_position())))
					.isRotate() == false) {
				...
				window.end = true;
				window.initialRotation = label.getAngle();
				((District) jackgame.getBoard().getCell(window.transformCoordToList(label.getMatrice_position())))
						.setRotate(true);
			} else {
		...
		// called at the end of swap action
		if (label.getMatrice_position() == window.currentRotable && window.end) {

			window.rotateImage(label, (label.getAngle() % 6.28 + 1.57));
			if (window.initialRotation == label.getAngle()) {
				window.rotateImage(label, (label.getAngle() % 6.28 + 1.57));
			}
			jackgame.rotate(window.RadianToOrientation(label.getAngle()),
					window.transformCoordToList(window.currentRotable), window);
		}
		
        //called at the end of detective action
		if ((label.getPath().contains("USED") && (label.getMatrice_position()[0] == 4))
				|| ((label.getMatrice_position()[0] == 5) && label.getPath().contains("USED"))) {
			jackgame.moveDetectiveToken(window.detectiveToMove, label.getCellcountToMove(), window, jackgame);
			window.actionPlaying = false;
		}
		
		// called if we click on a draw_card action
		if (label.getMatrice_position()[0] == 3 && window.drawCard) {
			window.drawCard = false;
			window.actionPlaying = false;
			window.hideCard();
			window.changeTurn(window, jackgame);
		}
	...
	// called if we click on a moveDetective action, show inspector next location
	public void transformCoord(int[] coord, int cellCount) {
		if (coord[0] == 4) {
			for (int x = 0; x < 3; x++) {
				if (window.listemilieuTwo[9 + (coord[1] - 1) * 3 + x].getPath()
						.equals(System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_.png")) {
					if (window.listemilieuTwo[9 + (coord[1] - 1) * 3 + 1].getPath()
							.equals(System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_.png")) {
						window.listemilieuTwo[9 + (coord[1] - 1) * 3 + 1].setIcon(window.reSize(
								new ImageIcon(System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_"
										+ window.detectiveToMove.toString() + "_USED.png"),
								window.allSize[4][0], window.allSize[4][1]));
						window.changePath(
								System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_"
										+ window.detectiveToMove + "_USED.png",
								"", window.listemilieuTwo[9 + (coord[1] - 1) * 3 + 1]);
						window.listemilieuTwo[9 + (coord[1] - 1) * 3 + 1].setCellcountToMove(cellCount);
						break;
					} else {
						window.listemilieuTwo[9 + (coord[1] - 1) * 3 + x].setIcon(window.reSize(
								new ImageIcon(System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_"
										+ window.detectiveToMove.toString() + "_USED.png"),
								window.allSize[4][0], window.allSize[4][1]));
						window.changePath(
								System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_"
										+ window.detectiveToMove + "_USED.png",
								"", window.listemilieuTwo[9 + (coord[1] - 1) * 3 + x]);
			...
			for (int i = 0; i < 3; i++) {
				if (window.listemilieuTwo[(coord[1] - 1) * 3 + i].getPath()
						.equals(System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_.png")) {
					if (window.listemilieuTwo[(coord[1] - 1) * 3 + 1].getPath()
							.equals(System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_.png")) {
						window.listemilieuTwo[(coord[1] - 1) * 3 + 1].setIcon(window.reSize(
								new ImageIcon(System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_"
										+ window.detectiveToMove.toString() + "_USED.png"),
								window.allSize[4][0], window.allSize[4][1]));
						window.changePath(
								System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_"
										+ window.detectiveToMove + "_USED.png",
								"", window.listemilieuTwo[(coord[1] - 1) * 3 + 1]);
						window.listemilieuTwo[(coord[1] - 1) * 3 + 1].setCellcountToMove(cellCount);
						break;
					} else {
						window.listemilieuTwo[(coord[1] - 1) * 3 + i].setIcon(window.reSize(
								new ImageIcon(System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_"
										+ window.detectiveToMove.toString() + "_USED.png"),
								window.allSize[4][0], window.allSize[4][1]));
						window.changePath(
								System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_"
										+ window.detectiveToMove + "_USED.png",
								"", window.listemilieuTwo[(coord[1] - 1) * 3 + i]);
			...
			for (int i = 0; i < 3; i++) {
				if (window.listemilieuThree[9 + (coord[0] - 1) * 3 + i].getPath()
						.equals(System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_.png")) {
					if (window.listemilieuThree[9 + (coord[0] - 1) * 3 + 1].getPath()
							.equals(System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_.png")) {
						window.listemilieuThree[9 + (coord[0] - 1) * 3 + 1].setIcon(window.reSize(
								new ImageIcon(System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_"
										+ window.detectiveToMove.toString() + "_USED.png"),
								window.allSize[4][0], window.allSize[4][1]));
						window.changePath(
								System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_"
										+ window.detectiveToMove + "_USED.png",
								"", window.listemilieuThree[9 + (coord[0] - 1) * 3 + 1]);
						window.listemilieuThree[9 + (coord[0] - 1) * 3 + 1].setCellcountToMove(cellCount);
						break;
					} else {
						window.listemilieuThree[9 + (coord[0] - 1) * 3 + i].setIcon(window.reSize(
								new ImageIcon(System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_"
										+ window.detectiveToMove.toString() + "_USED.png"),
								window.allSize[4][0], window.allSize[4][1]));
						window.changePath(
								System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_"
										+ window.detectiveToMove + "_USED.png",
								"", window.listemilieuThree[9 + (coord[0] - 1) * 3 + i]);
			...
			for (int i = 0; i < 3; i++) {
				if (window.listemilieuThree[(coord[0] - 1) * 3 + i].getPath()
						.equals(System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_.png")) {
					if (window.listemilieuThree[(coord[0] - 1) * 3 + 1].getPath()
							.equals(System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_.png")) {
						window.listemilieuThree[(coord[0] - 1) * 3 + 1].setIcon(window.reSize(
								new ImageIcon(System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_"
										+ window.detectiveToMove.toString() + "_USED.png"),
								window.allSize[4][0], window.allSize[4][1]));
						window.changePath(
								System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_"
										+ window.detectiveToMove + "_USED.png",
								"", window.listemilieuThree[(coord[0] - 1) * 3 + 1]);
						window.listemilieuThree[(coord[0] - 1) * 3 + 1].setCellcountToMove(cellCount);
						break;
					} else {
						window.listemilieuThree[(coord[0] - 1) * 3 + i].setIcon(window.reSize(
								new ImageIcon(System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_"
										+ window.detectiveToMove.toString() + "_USED.png"),
								window.allSize[4][0], window.allSize[4][1]));
						window.changePath(
								System.getProperty("user.dir") + "\\resources\\images\\tokens\\DETECTIVE_"
										+ window.detectiveToMove + "_USED.png",
								"", window.listemilieuThree[(coord[0] - 1) * 3 + i]);
